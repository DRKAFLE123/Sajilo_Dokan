...
export default function CustomerChatPage() {
  ...
  const targetShopId = searchParams.get('shop');
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      router.push('/login?redirect=/chat' + (targetShopId ? `?shop=${targetShopId}` : ''));
    }
  }, [isLoading, isAuthenticated, router, targetShopId]);

  const fetchConversations = async () => {
    try {
      const res = await api.get('/conversations/?role=customer');
      const fetchedConvos = Array.isArray(res.data) ? res.data : (res.data.results || []);
      setConversations(fetchedConvos);

      // If we have a target shop from URL, find or create the conversation
      if (targetShopId && isInitializing) {
        const existing = fetchedConvos.find((c: Conversation) => c.shop.id.toString() === targetShopId);
        if (existing) {
          setActiveConversation(existing);
        } else {
          // Create conversation
          const createRes = await api.post('/conversations/', { shop_id: parseInt(targetShopId) });
          setConversations([...fetchedConvos, createRes.data]);
          setActiveConversation(createRes.data);
        }
        setIsInitializing(false);
      } else if (!activeConversation && fetchedConvos.length > 0) {
        setActiveConversation(fetchedConvos[0]);
        setIsInitializing(false);
      } else if (activeConversation) {
        // Update active conversation data silently during polling
  ...
  if (isLoading || isInitializing) {
    return <div className="flex-1 flex items-center justify-center min-h-[60vh]">Loading chat...</div>;
  }

  return (
    <div className="flex-1 flex flex-col h-[calc(100dvh-64px)] max-h-[calc(100dvh-64px)] overflow-hidden bg-gray-50">
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col p-2 sm:p-4 lg:p-6 overflow-hidden min-h-0">
        <h1 className="text-2xl font-bold text-gray-900 mb-4 shrink-0 px-2 sm:px-0">Messages</h1>
        
        <div className="flex-1 bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden flex min-h-0">
        {/* Sidebar */}
        <div className="w-1/3 border-r border-gray-200 flex flex-col">
          <div className="p-4 border-b border-gray-200 bg-gray-50">
            <h2 className="font-semibold text-gray-700">Shops</h2>
          </div>
              ...
              conversations.map((conv) => (
                ...
                >
                  <div className="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 flex-shrink-0">
                    {conv.shop.logo ? (
                      <img src={conv.shop.logo} alt={conv.shop.name} className="h-10 w-10 rounded-full object-cover" />
                    ) : (
                      <Store size={20} />
                    )}
                  </div>
                  <div className="flex-1 overflow-hidden">
                    <div className="flex justify-between items-center mb-1">
                      <p className="font-semibold text-gray-900 truncate">{conv.shop.name}</p>
                      {conv.unread_count > 0 && (
                ...
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 flex-shrink-0">
                    {activeConversation.shop.logo ? (
                      <img src={activeConversation.shop.logo} alt={activeConversation.shop.name} className="h-10 w-10 rounded-full object-cover" />
                    ) : (
                      <Store size={20} />
                    )}
                  </div>
                  <div>
                    <h3 className="font-bold text-gray-900">{activeConversation.shop.name}</h3>
